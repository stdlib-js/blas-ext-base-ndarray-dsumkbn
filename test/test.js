...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( dsumkbn.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function computes the sum of all elements in a one-dimensional ndarray using improved Kahan–Babuška algorithm', function test( t ) {
	var x;
	var v;

	x = new Float64Array( [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ] );
	v = dsumkbn( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float64Array( [ -4.0, -5.0 ] );
	v = dsumkbn( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, -9.0, 'returns expected value' );

	x = new Float64Array( [ -0.0, 0.0, -0.0 ] );
	v = dsumkbn( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isPositiveZero( v ), true, 'returns expected value' );

	x = new Float64Array( [ NaN ] );
	v = dsumkbn( [ vector( x, 1, 1, 0 ) ] );
